...
def get_var_struct_features(model):
    ...
    # Loop over the connected_constraints and perform the computations
    for i, constraint in enumerate(connected_constraints):
        # Get the degrees (non-zero elements of column in A)
        degrees = A[constraint, i]

        # If degrees is empty, append zero to all lists and continue to next iteration
        if degrees.size == 0:
            degrees = np.array([0])
        ...
        # As coefficients are same as degrees in this case, we can reuse the degrees list
        mean_coefficient.append(np.mean(degrees))
        std_coefficient.append(np.std(degrees))
        min_coefficient.append(np.min(degrees))
        max_coefficient.append(np.max(degrees))
...
if __name__ == "__main__":
    ...
    if not config["update"]:
        ...
        # # read the dataset
        with open("Data/coordination/coordination.pickle", "rb") as f:
    ...
    with open("Data/coordination/coordination.pickle", "wb") as f:
