...
int main(int, char **) {
    ...
    for (int s = 0; s < nbScenarios; s++) {
        ...
        for (int i = 0; i < nbStations; i++) {
            RealTimeChargingPower[s][i] = IloNumVarArray(env, nbTime, 0, IloInfinity);
        }
    }
    ...
    for (int s = 0; s < nbScenarios; s++) {
        ...
        for (int i = 0; i < nbStations; i++) {
            PredictedChargingPower[s][i] = IloNumVarArray(env, nbTime, 0, IloInfinity);
        }
    }
    ...
    for (int s = 0; s < nbScenarios; s++) {
        for (int t = t0 + 1; t < t0 + H; t++) {
            expr_powerbalance_realtime[s][t] = RealTimeSolarOutput[s][t] - RealTimeSolarOutput[s][t];
